...
| Mac | 1440 × 900 (2880 × 1800 на Retina-экране) | окно Mac-версии |
...
каждого языка. Снимайте после замены картинок, а не до.

Mac снимается тем же скриптом в `docs/store/<язык>/Mac/`: Debug-сборка под
отдельным bundle ID (`…SashasPazzle.screenshots`), поэтому ваши сохранения и
фото на этом Mac не трогаются. Окно 1440 × 900 pt: на обычном мониторе это
1440 × 900 px, на Retina — 2880 × 1800, оба размера App Store принимает.
Только Mac, без симуляторов: `SIMULATORS= Scripts/store-screenshots.sh ru`.
