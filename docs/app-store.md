...
`complete.caf`, а также две мелодии: `music-library.m4a` и `music.m4a`. Их делает
`swift Scripts/make-sounds.swift`: он кладёт на Рабочий стол по три варианта
каждого эффекта и черновики обеих мелодий с `.mid` для Logic. Выбранный
вариант переименуйте и замените файл в папке.
...
| `music-library.m4a` | библиотека и выбор картинки: спокойная, ре мажор, 3/4 |
| `music-piano.m4a` | стол, вариант «Фортепиано» (по умолчанию): соль мажор, 3/4 |
| `music.m4a` | стол, вариант «Вибрафон»: фа мажор, вибрафон и фортепиано |
...
- в библиотеке и на экране настройки пазла звучит `music-library`, на
  столе — мелодия, выбранная в «Настройки → Музыка за столом»; при переходе одна перетекает в другую за
  `Feedback.crossfade` (2 с). Каждая мелодия продолжает с того места, где
  затихла; при сворачивании обе на паузе;
...
- в настройках два переключателя: «Звуки» и «Фоновая музыка». Второй
  появляется, только когда в сборке есть хотя бы одна мелодия;
